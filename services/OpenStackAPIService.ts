import {
  ...
  CinderVolumeRsp,
  NeutronNetworksRsp,
  NeutronFloatingIPsRsp,
  GlanceImagesRsp,
  NeutronSecurityGroupsRsp,
  NeutronSecurityGroupRsp,
  NovaFlavorsRsp,
  NovaKeyPairsRsp,
  QuotaSetRsp,
  NeutronSecurityGroupRuleRsp,
  CinderQuotaSet,
  NeutronQuota,
  NovaQuotaSet,
  NovaRawServer,
  NovaRawServerAddressEntry,
  Subnet,
  Port,
  Router, // Import new types
  NeutronSubnetRsp,
  NeutronSubnetsRsp, // Import new response types
  NeutronPortRsp,
  NeutronPortsRsp,
  NeutronRouterRsp,
  NeutronRoutersRsp,
...
export const getServiceEndpoint = (
  ...
  let endpoint = service.endpoints.find(
    (e) =>
      e.interface === serviceInterface && (region ? e.region === region : true)
  );
...
async function apiClient<T>(
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      fullErrorMessage +=
        " Authentication error (401): Your session might have expired or token is invalid. Please try logging out and logging back in. This could also be a CORS issue if X-Auth-Token is not properly exposed by the server. Check browser Network tab for details.";
    } else {
      fullErrorMessage +=
        " Check browser Network tab for details. Ensure OpenStack service endpoints have CORS configured if this is a cross-origin request.";
    }
...
const buildFullSvcPath = (
  ...
  const normalizedResourcePath = resourcePath.startsWith("/")
    ...
    : resourcePath;
  if (baseUrl.endsWith(`/${versionSegment}`)) {
    // e.g., baseUrl is http://host/network/v2.0
...
const mapRawServerToInstance = (rawServer: NovaRawServer): Instance => {
  ...
  if (vmState === "SHELVED" || vmState === "SHELVED_OFFLOADED") {
    ...
    powerState = "Paused";
  } else if (vmState === "active" || vmState === "RUNNING") {
    // 'ACTIVE' usually means running
    switch (rawServer.power_state) {
      ...
      case 3:
        powerState = "Paused";
        break; // Should be covered by vmState check but good fallback
      case 4:
        powerState = "Shutoff";
        break; // Should be covered by vmState check
      default:
        powerState = "Running"; // If vm_state is ACTIVE, assume Running unless power_state says otherwise
    }
  } else {
    ...
    switch (rawServer.power_state) {
      case 0:
        powerState = "Shutoff";
        break; // NOSTATE, often maps to Shutoff in practice if vm_state unknown
      case 1:
        powerState = "Running";
        break;
      case 3:
      ...
      default:
        console.warn(
          `Unknown OpenStack power_state code: ${rawServer.power_state} and ambiguous vm_state: ${vmState} for instance ${rawServer.id}. Mapping to 'Error'.`
        );
        powerState = "Error";
        break;
    }
  ...
  return {
    ...
    flavor: {
      id: rawServer.flavor.id,
      name: undefined /* Consider fetching flavor details if needed */,
    },
    image: {
      id: rawServer.image.id,
      name: undefined /* Consider fetching image details if needed */,
    },
    ipAddress: ipAddress,
    powerState: powerState, // More granular power state for UI display
    keyPair: rawServer.key_name || undefined,
    securityGroups: rawServer.security_groups
      ? rawServer.security_groups.map((sg) => sg.name)
      : [],
    ["os-extended-volumes:volumes_attached"]:
      rawServer["os-extended-volumes:volumes_attached"],
    ...
    locked:
      rawServer.locked === true ||
      String(rawServer.locked).toLowerCase() === "true", // Ensure boolean
    description: rawServer.description || null,
    hostname:
      rawServer["OS-EXT-SRV-ATTR:hostname"] ||
      rawServer["OS-EXT-SRV-ATTR:hypervisor_hostname"] ||
      "null", // Take hostname or hypervisor_hostname
...
export const fetchInstances = (
  ...
  apiClient<NovaServersRsp>(`${computeUrl}/servers/detail`, {
    method: "GET",
    token,
  }).then((data) => data.servers.map(mapRawServerToInstance));

export const fetchInstanceDetailsAPI = (
  token: string,
  computeUrl: string,
  instanceId: string
): Promise<Instance> =>
  apiClient<NovaServerRsp>(`${computeUrl}/servers/${instanceId}`, {
    method: "GET",
    token,
  }).then((data) => mapRawServerToInstance(data.server));

export const launchInstanceAPI = async (
  token: string,
  computeUrl: string,
  params: any
): Promise<Instance> => {
  const serverPayload = {
    server: {
      ...
      security_groups: params.securityGroupIds
        ? params.securityGroupIds.map((sgIdOrName: string) => ({
            name: sgIdOrName,
          }))
        : undefined,
      networks:
        params.networkIds && params.networkIds.length > 0
          ? params.networkIds.map((netId: string) => ({ uuid: netId }))
          : [{ uuid: "auto" }],
    },
  };
  return apiClient<NovaServerRsp>(`${computeUrl}/servers`, {
    method: "POST",
    token,
...
export const terminateInstanceAPI = (
  token: string,
  computeUrl: string,
  instanceId: string
): Promise<void> =>
  apiClient<void>(`${computeUrl}/servers/${instanceId}`, {
    method: "DELETE",
    token,
  });

export const controlInstancePowerAPI = (
  token: string,
  computeUrl: string,
  instanceId: string,
  action: "os-start" | "os-stop" | "reboot" /* 'reboot' needs type*/
): Promise<void> => {
  ...
  return apiClient<void>(`${computeUrl}/servers/${instanceId}/action`, {
    method: "POST",
    token,
    body: JSON.stringify(openstackAction),
  });
};

export const getInstanceConsoleUrlAPI = (
  token: string,
  computeUrl: string,
  instanceId: string,
  consoleType: "novnc" | "spice-html5" | "serial" = "novnc"
): Promise<{ console: { type: string; url: string } }> => {
  const payload: any = {};
  if (consoleType === "novnc") payload["os-getVNCConsole"] = { type: "novnc" };
  else if (consoleType === "spice-html5")
    payload["os-getSPICEConsole"] = { type: "spice-html5" };
  else if (consoleType === "serial")
    payload["os-getSerialConsole"] = { type: "serial" };
  else throw new Error(`Unsupported console type: ${consoleType}`);

  return apiClient<{ console: { type: string; url: string } }>(
    `${computeUrl}/servers/${instanceId}/action`,
    { method: "POST", token, body: JSON.stringify(payload) }
  );
};

export const shelveInstanceAPI = (
  token: string,
  computeUrl: string,
  instanceId: string
): Promise<void> => {
...
export const unshelveInstanceAPI = (
  token: string,
  computeUrl: string,
  instanceId: string
): Promise<void> => {
...
export const createInstanceSnapshotAPI = (
  token: string,
  computeUrl: string,
  instanceId: string,
  snapshotName: string
): Promise<void | { image_id: string }> => {
...
export const fetchVolumes = (
  ...
  apiClient<CinderVolumesRsp>(`${volumeUrl}/volumes/detail`, {
    method: "GET",
    token,
  }).then((data) => data.volumes);

export const createVolumeAPI = (
  token: string,
  volumeUrl: string,
  params: {
    name: string;
  ...
  return apiClient<CinderVolumeRsp>(`${volumeUrl}/volumes`, {
    method: "POST",
    token,
    body: JSON.stringify(payload),
...
export const deleteVolumeAPI = (
  token: string,
  volumeUrl: string,
  volumeId: string
): Promise<void> =>
  apiClient<void>(`${volumeUrl}/volumes/${volumeId}`, {
    method: "DELETE",
    token,
  });

export const attachVolumeAPI = (
  token: string,
  computeUrl: string,
  instanceId: string,
  volumeId: string
  ...
  return apiClient<any>(
    `${computeUrl}/servers/${instanceId}/os-volume_attachments`,
    { method: "POST", token, body: JSON.stringify(payload) }
  );
};

export const detachVolumeAPI = (
  token: string,
  computeUrl: string,
  instanceId: string,
  attachmentId: string
  ...
  return apiClient<void>(
    `${computeUrl}/servers/${instanceId}/os-volume_attachments/${attachmentId}`,
    { method: "DELETE", token }
  );
};

export const extendVolumeAPI = (
  token: string,
  volumeUrl: string,
  volumeId: string,
  newSize: number
...
export const createVolumeSnapshotAPI = (
  token: string,
  volumeUrl: string,
  volumeId: string,
  snapshotName: string,
  ...
  // Return type can be more specific if snapshot object is defined
...
export const fetchNetworks = (
  ...
  apiClient<NeutronNetworksRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "networks"),
    { method: "GET", token }
  ).then((data) =>
    data.networks.map((n) => ({ ...n, subnet_ids: n.subnets || [] } as Network))
  ); // Map to subnet_ids

export const fetchSubnetsAPI = (
  token: string,
  networkUrl: string,
  networkId?: string
): Promise<Subnet[]> => {
  const params = networkId ? { network_id: networkId } : {};
  return apiClient<NeutronSubnetsRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "subnets"),
    { method: "GET", token, params }
  ).then((data) => data.subnets);
};

export const fetchRoutersAPI = (
  token: string,
  networkUrl: string
): Promise<Router[]> =>
  apiClient<NeutronRoutersRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "routers"),
    { method: "GET", token }
  ).then((data) => data.routers);

export const fetchPortsAPI = (
  token: string,
  networkUrl: string,
  queryParams?: Record<string, string>
): Promise<Port[]> =>
  apiClient<NeutronPortsRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "ports"),
    { method: "GET", token, params: queryParams }
...
export const fetchFloatingIPs = (
  ...
  apiClient<NeutronFloatingIPsRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "floatingips"),
    { method: "GET", token }
  ).then((data) => data.floatingips);

export const allocateFloatingIPAPI = (
  token: string,
  networkUrl: string,
  poolNetworkId: string
): Promise<FloatingIP> => {
  const payload = { floatingip: { floating_network_id: poolNetworkId } };
  return apiClient<{ floatingip: FloatingIP }>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "floatingips"),
    { method: "POST", token, body: JSON.stringify(payload) }
  ).then((data) => data.floatingip);
};

export const releaseFloatingIPAPI = (
  token: string,
  networkUrl: string,
  fipId: string
): Promise<void> =>
  apiClient<void>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, `floatingips/${fipId}`),
    { method: "DELETE", token }
  );

export const associateFloatingIPAPI = (
  token: string,
  networkUrl: string,
  fipId: string,
  portId: string
...
export const disassociateFloatingIPAPI = (
  token: string,
  networkUrl: string,
  fipId: string
): Promise<FloatingIP> => {
...
export const fetchImages = (
  ...
  apiClient<GlanceImagesRsp>(
    buildFullSvcPath(imageUrl, GLANCE_API_VERSION, "images"),
    { method: "GET", token }
...
export const fetchSecurityGroups = (
  ...
  apiClient<NeutronSecurityGroupsRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "security-groups"),
    { method: "GET", token }
  ).then((data) =>
  ...
  ); // Normalize rules field

export const createSecurityGroupAPI = (
  token: string,
  networkUrl: string,
  name: string,
  description: string
  ...
  return apiClient<NeutronSecurityGroupRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "security-groups"),
    { method: "POST", token, body: JSON.stringify(payload) }
  ).then((data) => data.security_group);
};

export const deleteSecurityGroupAPI = (
  token: string,
  networkUrl: string,
  sgId: string
): Promise<void> =>
...
export const addSecurityGroupRuleAPI = (
  token: string,
  networkUrl: string,
  rule: Omit<SecurityGroupRule, "id" | "project_id">
): Promise<SecurityGroupRule> => {
  const payload = { security_group_rule: rule };
  return apiClient<NeutronSecurityGroupRuleRsp>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, "security-group-rules"),
    { method: "POST", token, body: JSON.stringify(payload) }
  ).then((data) => data.security_group_rule);
};

export const deleteSecurityGroupRuleAPI = (
  token: string,
  networkUrl: string,
  ruleId: string
): Promise<void> =>
...
export const fetchFlavors = (
  ...
  apiClient<{ flavors: any[] }>(`${computeUrl}/flavors/detail`, {
    method: "GET",
    token,
  }).then((data) =>
    data.flavors.map((rawFlavor) => {
      ...
      return {
        ...
        ["OS-FLV-EXT-DATA:ephemeral"]:
          rawFlavor["OS-FLV-EXT-DATA:ephemeral"] || 0, // Default ephemeral to 0 if not present
...
export const fetchKeyPairs = (
  ...
  apiClient<NovaKeyPairsRsp>(`${computeUrl}/os-keypairs`, {
    method: "GET",
    token,
  }).then((data) => data.keypairs.map((kpItem) => kpItem.keypair)); // Correctly access the nested keypair object

// Quotas (Nova, Cinder, Neutron)
export const fetchNovaQuotas = async (
  token: string,
  computeUrl: string,
  projectId: string
): Promise<NovaQuotaSet> => {
  const data = await apiClient<QuotaSetRsp>(
    `${computeUrl}/os-quota-sets/${projectId}`,
    { method: "GET", token }
  );
  return data.quota_set as unknown as NovaQuotaSet;
};

export const fetchCinderQuotas = async (
  token: string,
  volumeUrl: string,
  projectId: string
): Promise<CinderQuotaSet> => {
  const data = await apiClient<{ quota_set: CinderQuotaSet }>(
    `${volumeUrl}/os-quota-sets/${projectId}`,
    { method: "GET", token }
  );
  return data.quota_set;
};

export const fetchNeutronQuotas = async (
  token: string,
  networkUrl: string,
  projectId: string
): Promise<NeutronQuota> => {
  const data = await apiClient<{ quota: NeutronQuota }>(
    buildFullSvcPath(networkUrl, NEUTRON_API_VERSION, `quotas/${projectId}`),
    { method: "GET", token }
  );
  return data.quota;
};
...
export const fetchAllQuotas = async (
  ...
  if (!projectId) {
    console.warn(
      "fetchAllQuotas: Project ID is undefined. Returning empty quota list."
    );
  ...
  const volumeUrl =
    getServiceEndpoint(serviceCatalog, "volumev3") ||
    getServiceEndpoint(serviceCatalog, "volumev2") ||
    getServiceEndpoint(serviceCatalog, "volume");
  ...
  try {
    if (computeUrl) {
      const novaData: NovaQuotaSet = await fetchNovaQuotas(
        token,
        computeUrl,
        projectId
      );
    ...
    console.warn("Failed to fetch Nova quotas:", e);
  }

  try {
    if (volumeUrl) {
      const cinderData: CinderQuotaSet = await fetchCinderQuotas(
        token,
        volumeUrl,
        projectId
      );
    ...
    console.warn("Failed to fetch Cinder quotas:", e);
  }

  try {
    if (networkUrl) {
      const neutronData: NeutronQuota = await fetchNeutronQuotas(
        token,
        networkUrl,
        projectId
      );
    ...
    console.warn("Failed to fetch Neutron quotas:", e);
  }
